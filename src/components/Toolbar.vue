<template>
  <div class="flex items-center justify-between py-1 text-sm border-neutral-300">
    <div class="flex py-1 text-center" v-if="!searchQuery.length">
        <div class="mx-1.5"
            v-if="visibility('new_folder') && !hidden('new_folder')"
          ...
          <svg xmlns="http://www.w3.org/2000/svg"
               class="w-4 h-4 m-auto cursor-pointer md:h-6 md:w-6 stroke-gray-500 hover:stroke-cyan-700 dark:stroke-gray-400 dark:hover:stroke-gray-300" fill="none" viewBox="0 0 24 24" stroke="none" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 10.5v6m3-3H9m4.06-7.19l-2.12-2.12a1.5 1.5 0 00-1.061-.44H4.5A2.25 2.25 0 002.25 6v12a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18V9a2.25 2.25 0 00-2.25-2.25h-5.379a1.5 1.5 0 01-1.06-.44z" />
          </svg>
        </div>

        <div class="mx-1.5"
              v-if="visibility('new_file') && !hidden('new_file')"
        ...
        <div class="mx-1.5"
              v-if="visibility('rename') && !hidden('rename')"
          ...
          <svg xmlns="http://www.w3.org/2000/svg"
               ...
               class="w-4 h-4 m-auto md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="none" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
          </svg>
        </div>

        <div class="mx-1.5"
              v-if="visibility('delete') && !hidden('delete')"
        ...
        <div class="mx-1.5"
              v-if="visibility('upload') && !hidden('upload')"
          ...
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 m-auto cursor-pointer md:h-6 md:w-6 stroke-gray-500 hover:stroke-cyan-700 dark:stroke-gray-400 dark:hover:stroke-gray-300" fill="none" viewBox="0 0 24 24" stroke="none" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
          </svg>
        </div>

        <template v-if="visibility('archive') && !hidden('archive')">
          <div class="mx-1.5" v-if="selectedItems.length == 1 && selectedItems[0].mime_type == 'application/zip'"
            ...
            <svg xmlns="http://www.w3.org/2000/svg"
                 :class="(selectedItems.length) ? 'cursor-pointer stroke-gray-500 hover:stroke-cyan-700 dark:stroke-gray-400 dark:hover:stroke-gray-300' : 'stroke-gray-200  dark:stroke-gray-700'"
                 class="w-4 h-4 m-auto md:h-8 md:w-8" fill="none" viewBox="0 0 24 24" stroke="none" stroke-width="1.5">
    ...
    <div class="flex text-center" v-else>
      <div class="pl-2"> {{ t('Search results for') }} <span class="px-2 py-1 text-xs bg-gray-200 rounded dark:bg-gray-700">{{ searchQuery }}</span></div>
       <svg
...
<!--               class="w-6 h-6 m-auto cursor-pointer stroke-sky-500 fill-sky-100 hover:stroke-sky-600 dark:stroke-gray-400 dark:fill-gray-400/20 dark:hover:stroke-gray-300">-->
<!--                <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />-->
<!--          </svg>-->
<!--        </div>-->

         <div  v-if="visibility('fullscreen') && !hidden('fullscreen')" class="mx-1.5" :aria-label="t('Toggle Full Screen')" data-microtip-position="bottom-left" role="tooltip"
               @click="setFullScreen">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 m-auto cursor-pointer md:h-6 md:w-6 stroke-gray-500 hover:stroke-cyan-700 dark:stroke-gray-400 dark:hover:stroke-gray-300" fill="none" viewBox="0 0 24 24" stroke="none" stroke-width="1.5">
        ...
        <div class="mx-1.5"
              v-if="visibility('view') && !hidden('view')"
          ...
          <svg xmlns="http://www.w3.org/2000/svg"
               ...
               class="w-6 h-6 m-auto md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="none" stroke-width="1.5">
...
function visibility(tool) {
  return props.toolbarSets.includes(tool)
}

function hidden(tool) {
  return props.hiddenToolbarSets.includes(tool)
}
