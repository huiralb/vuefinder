<template>
  <div class="relative flex flex-col flex-auto overflow-hidden">
    <div v-if="view=='list' || searchQuery.length" class="grid grid-cols-12 text-xs border-b border-gray-200 select-none border-neutral-300 dark:border-gray-700">
        <div @click="sortBy('basename')" class="flex items-center col-span-7 py-1 pl-1 leading-6 hover:bg-neutral-100 bg-neutral-50 dark:bg-gray-800 dark:hover:bg-gray-700/10">
          {{ t('Name') }}
            <v-f-sort-icon :direction="sort.order=='asc'? 'down': 'up'" v-show="sort.active && sort.column=='basename'" />
        </div>
        <div v-if="!searchQuery.length" @click="sortBy('file_size')" class="flex items-center justify-center col-span-2 py-1 leading-6 border-l border-r hover:bg-neutral-100 bg-neutral-50 dark:bg-gray-800 dark:hover:bg-gray-700/10 dark:border-gray-700">
          {{ t('Size') }}
          <v-f-sort-icon :direction="sort.order=='asc'? 'down': 'up'"  v-show="sort.active && sort.column=='file_size'" />
        </div>
        <div v-if="!searchQuery.length" @click="sortBy('last_modified')" class="flex items-center justify-center col-span-3 py-1 leading-6 hover:bg-neutral-100 bg-neutral-50 dark:bg-gray-800 dark:hover:bg-gray-700/10">
          {{ t('Date') }}
          <v-f-sort-icon :direction="sort.order=='asc'? 'down': 'up'"  v-show="sort.active && sort.column=='last_modified'" />
        </div>
        <div v-if="searchQuery.length" @click="sortBy('path')" class="flex items-center justify-center col-span-5 py-1 leading-6 border-l hover:bg-neutral-100 bg-neutral-50 dark:bg-gray-800 dark:hover:bg-gray-700/10 dark:border-gray-700">
          {{ t('Filepath') }}
    ...
    <div class="absolute">
      <div ref="dragImage"  class="absolute -z-50 -top-96">
        <svg xmlns="http://www.w3.org/2000/svg" class="absolute z-10 w-6 h-6 m-auto md:h-12 md:w-12 stroke-neutral-500 fill-white dark:fill-gray-700 dark:stroke-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
    ...
    <div
      ...
      <div
          ...
          <div class="grid items-center grid-cols-12">
            <div class="flex items-center col-span-7">
              <svg v-if="item.type == 'dir'" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-neutral-500 fill-sky-500 stroke-sky-500 dark:fill-slate-500 dark:stroke-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
              </svg>
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                <path stroke-linecap="round" stroke-linejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
              <span class="overflow-hidden overflow-ellipsis whitespace-nowrap">{{item.basename }}</span>
            </div>
            <div class="col-span-5 overflow-hidden overflow-ellipsis whitespace-nowrap">{{ item.path }}</div>
          </div>
      </div>
      ...
      <div draggable="true"
          ...
          <div class="grid items-center grid-cols-12">
            ...
            <div class="col-span-3 overflow-hidden overflow-ellipsis whitespace-nowrap">{{ datetimestring(item.last_modified) }}</div>
          </div>
      </div>
      ...
      <div draggable="true"
          ...
          <div>
            <div class="relative">
              <svg v-if="item.type == 'dir'" xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 m-auto md:h-12 md:w-12 fill-sky-500 stroke-sky-500 dark:fill-slate-500 dark:stroke-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
              </svg>
              <img class="h-10 m-auto lazy md:h-12" v-else-if="(item.mime_type ?? '').startsWith('image')" :data-src="getImageUrl(adapter.value, item.path)"  :alt="item.basename">
              <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 m-auto md:h-12 md:w-12 text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                <path stroke-linecap="round" stroke-linejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
              </svg>
              <div class="absolute hidden w-full text-center md:block top-1/2 text-neutral-500" v-if="!(item.mime_type ?? '').startsWith('image') && item.type != 'dir'">{{ ext(item.extension) }}</div>
            </div>
